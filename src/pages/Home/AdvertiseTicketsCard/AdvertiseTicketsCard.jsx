...
const AdvertiseTicketsCard = () => {
  ...
  if (!advertiseTickets || advertiseTickets.length === 0) return null;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-base-content/70 mt-4 max-w-2xl mx-auto">
            Handpicked premium journeys — specially promoted by our admins just
            for you!
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8">
          {advertiseTickets.map((ticket) => (
            <div
              key={ticket._id}
              className="group relative bg-base-100 rounded-2xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-500 border border-base-300 transform hover:-translate-y-2 flex flex-col"
            >
              {/* Image */}
              <div className="relative h-48 md:h-56 overflow-hidden">
                ...
                <div className="absolute bottom-3 left-3 text-white z-10">
                  ...
                  <p className="text-xs md:text-sm opacity-90">per seat</p>
                </div>

                {/* Featured Badge */}
                <div className="absolute top-3 right-3 bg-gradient-to-r from-[#667eea] to-[#764ba2] text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg">
                  Featured
                </div>
              </div>
              ...
              <div className="p-4 flex flex-col flex-1 justify-between">
                <div>
                  {/* Title */}
                  <h3 className="text-lg md:text-2xl font-bold text-primary line-clamp-1">
                    {ticket.title}
                  </h3>

                  {/* From → To */}
                  <p className="text-sm md:text-lg font-medium flex items-center gap-2 mt-1">
                ...
                <button
                  ...
                  className="btn w-full mt-4 bg-gradient-to-r from-[#667eea] to-[#764ba2] text-white font-bold text-lg hover:shadow-2xl transform hover:scale-105 transition-all duration-300 border-0"
                >
                  Book Now
                </button>
              </div>
